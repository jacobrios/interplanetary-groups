...
describe("resetInviteToken", () => {
  ...
  async function makeGroup() {
    ...
    const founderAuthId = `test-reset-founder-${stamp}`
    const founder = await prisma.user.create({
      data: { name: "[TEST] Reset Founder", supabaseAuthId: founderAuthId },
    })
  ...
  it("rejects a non-founder", async () => {
    ...
    const stamp = `${Date.now()}-${Math.floor(Math.random() * 1e6)}`
    const memberAuthId = `test-reset-member-${stamp}`
    const member = await prisma.user.create({
      data: { name: "[TEST] Reset Member", supabaseAuthId: memberAuthId },
    })
    ...
    await expect(
      resetInviteToken({ supabaseAuthId: memberAuthId, groupId: group.id })
  ...
  it("rejects an unknown group", async () => {
    const { founderAuthId } = await makeGroup()
    await expect(
      resetInviteToken({ supabaseAuthId: founderAuthId, groupId: "no-such-group" })
