...
import { NO_PLANS_REPLY, PAST_TIME_REPLY } from "../change-copy"
...
describe("planChange", () => {
  it("clear time request in a solo group moves, announcement included", () => {
    const p = planChange(change(), TARGET, [TARGET], "Sam", 1, ZONE, NOW)
  ...
  it("probable intent asks instead of moving", () => {
    const p = planChange(change({ intentClear: false }), TARGET, [TARGET], "Sam", 5, ZONE, NOW)
  ...
  it("a clearly asked non-time request gets the honest decline", () => {
    const p = planChange(change({ requestedFields: ["venue"] }), TARGET, [TARGET], "Sam", 5, ZONE, NOW)
  ...
  it("a compound day-and-time request declines rather than acting on half", () => {
    const p = planChange(change({ requestedFields: ["day", "time"] }), TARGET, [TARGET], "Sam", 5, ZONE, NOW)
    if (p.action !== "reply") throw new Error(`expected reply, got ${p.action}`)
  ...
  it("a resolved instant in the past gets the honest reply when clearly asked", () => {
    ...
    const p = planChange(change({ requestedTime: "08:00" }), sameDayTarget, [sameDayTarget], "Sam", 5, ZONE, NOW)
    if (p.action !== "reply") throw new Error(`expected reply, got ${p.action}`)
