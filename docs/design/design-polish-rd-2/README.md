...
2. **Carousel chrome** — the group home's pinned event region. The shipped chrome is interim (dots
   ...
   single-card case.
3. **Pending strip · separation treatment, plus a quiet teal band** — the collapsed one-line strip between the event card and
...
4. **Finish-layer pilot (group home only)** — **declined.** Kept from it: tabular numerals in tallies
   and the grounded composer, both folded into the base build. The before/after pair and the pilot
   stylesheet remain in this bundle as the record.
...
- **Copy block** (`.fd-copy`, `margin-top: auto`) — the mark holds the top of the screen, the copy and
  the action group at the bottom, and the space between them absorbs longer translations.
  - **Eyebrow** — `--type-eyebrow` 13px, uppercase, letter-spacing `.18em`, weight 700,
    `--text-faint`. Copy: "Interplanetary Groups".
  - **Headline** — `--type-display` 28px, Hanken Grotesk 800, `--leading-tight` 1.15, letter-spacing
    ...
    Copy: "Casual plans shouldn't need a wedding planner."
  - **Lede** — `--type-body` 17px / `--leading-normal` 1.5, `--text-secondary`, `margin-top: 12px`,
    `text-wrap: pretty`. Copy: "But the other option is 'show up if you want,' and then nobody does.
    Orbit picks a day, asks the group, and keeps track of who's in." (straight double quotes around
    the quoted phrase in the shipped copy).
- **Primary action** — the one teal element on the screen. Full width, `min-height: 52px`,
  ...
  arrow icon 1.05em at `--action-ink`, `padding: .5em 1.2em`, `.fd-foot` `padding-top: 22px`.
  Copy: "Start your group".
...
nothing truncates. Total content is one headline, one short lede, one action and the small note:
no feature list, no second lede, no marketing sections.
...
The event card remains the only raised, bordered, shadowed object on the screen.

### 03b — The band, washed with action teal
**Amended constraint:** the event card is no longer the only teal on the screen. The pending strip is a
real action surface (decisions waiting on this member), so its band carries a translucent wash of the
action teal instead of sitting colourless.

- `.pd-strip.pd-sep.tint-a` — **subtle**, `background-color: rgba(24,188,203,.07)` ≈ #1d2931 over
  `--surface-base`. Reads as a tint you notice second, after the card.
- `.pd-strip.pd-sep.tint-b` — **stronger**, `background-color: rgba(24,188,203,.13)` ≈ #23343d over
  `--surface-base`. Reads as teal on first glance; about as far as it can go before it competes with
  the card's button.

Both are drawn side by side in the reference file. **Ship one, delete the other** — it is a one-class
change and nothing else moves.

Hard requirements, all met by both strengths:
- The wash is a low-alpha **surface**, not a fill. Against a solid `--action` button at #18bccb it is a
  fraction of the chroma, and the band still sits darker than the card above it. The band never takes
  the `--action-ink` label treatment, because it is not a button.
- **Counts stay the only bright text** in the band: `--text-primary` weight 700 numerals,
  `--text-secondary` words. The wash does not lighten the label.
- **Geometry unchanged from 03:** full-bleed hairlines top and bottom, `padding: 12px 20px`, 20px feed
  gutter, one-line height, 14px of air above, clock icon `--text-secondary`, chevron `--text-faint`,
  no radius, no border box, no shadow.
- **Greyscale-safe.** Both reduce to a slightly lighter grey band; the sentence carries the meaning.
- The wash is on the **strip only**. The expanded panel keeps `--surface-base`, so opening the panel
  does not flood the screen with teal.

### 04 — Finish-layer pilot (group home) · DECLINED
**Verdict: declined.** Two moves are kept and folded into the base build, and are now in
`round4-base.css` (unscoped, applying to every screen): **tabular numerals** in tallies
(`font-variant-numeric: tabular-nums` on `.gh-statusline`, `.pd-striptxt`, `.gh-msg .gtally`) and the
**grounded composer** (`linear-gradient(0deg, rgba(0,0,0,.34), rgba(0,0,0,0))` under `.gh-pin`).
Everything else below is kept as the record of what was piloted and rejected. `finish-layer.css` is
**not part of the build**; do not load it.
...
- **Band wash:** not state. A single class on the strip, chosen once at build time.
...
No colour values are added. The band wash is `--action` #18bccb at 7% or 13% alpha over
`--surface-base`.

Colour independence: dot state is width + brightness; card status is a bright count plus a word label
("4 In · 1 Out · 4 TBD"); the pending band's teal wash is decoration over a sentence that already
carries the meaning. Nothing this round encodes meaning in hue.
...
- `round4-base.css` — items 01, 02, 03, the 03b band washes, and the two moves kept from the declined
  pilot. **Source of truth for measurements.** The `.gh-rail.at2` / `.at3` block is mock-only: static
  stand-ins for two scroll positions on the board, not production.
- `finish-layer.css` — the declined pilot, every rule scoped to `.fx`. Kept for the record; **not part
  of the build**.
...
Load order: `walkthrough.css` → `pending-surface.css` → `round4-base.css`. (`finish-layer.css` is
declined and not loaded; the reference HTML still links it so the before/after record renders.)
