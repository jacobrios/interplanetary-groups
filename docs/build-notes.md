...
- **A second web address silently orphans every session on the first one, and the people behind them come back as duplicates (registered 24 Aug 2026, from the owner's own experience on production).** Sessions live in cookies, and cookies belong to one exact host, so `interplanetary-groups.vercel.app` and `interplanetarygroups.com` hold entirely separate sessions for the same person. Adding the custom domain did not migrate anyone; it stranded them. The owner hit this himself and read it as having lost access, which is the kinder of the two possible readings. The harsher one is what actually happens next: with no session on the new host, tapping the invite link joins them again as a *second* person, the group holds two of them, their earlier answers belong to an identity nobody can reach, and every count is quietly wrong, with no error anywhere. This is the same defect the 23 Aug postscript records for a cleared cache, but a domain change fires it for **everyone at once** rather than one person at a time, and it fires silently on what looks like an ordinary improvement. Two consequences worth separating: until email sign-in exists, exactly one host should ever be shared, and after email sign-in exists this stops mattering, because identity would no longer be the cookie. It is also worth deciding whether the old host should redirect to the new one rather than continue serving a second copy of the product. Sequencing is the owner's call.

**Recurred 2 September 2026, in QA rather than in production, same shape exactly.** The owner tried to eyeball the `/groups` legal-footer fix (ready-for-other-peoples-data slice) on his phone, and the links he was given pointed at the Mac's LAN address, a different origin from whatever host his existing session cookie belonged to. His session did not travel, so he arrived with zero groups and was redirected to the front door instead of `/groups`. Nothing was wrong with the code; the footer fix is confirmed by its own tests (`src/components/__tests__/YourGroupsScreen.test.tsx`, `src/app/groups/__tests__/page.test.tsx`), and this entry's own reasoning already predicted the exact failure. What it means in practice: **any QA handoff link must be checked against which host the tester's session actually lives on**, not just copied from wherever the dev server happens to be listening, or the tester gets bounced to a logged-out screen and reads it as a bug.

- **No safe place to make a mess (registered 24 Aug 2026, from the owner's question, which was the right question).** The dev-test database is reachable only from the owner's own machine and the phones on his wifi, because the only thing that ever points at it is the local dev server. There is no deployed copy of the product wired to it. The practical effect is that the live site is currently the only realistic surface for trying anything from another device, with another person, or from anywhere else, which makes the production database the default test bed by absence rather than by choice. The shape of the answer is a preview deployment on the hosting platform, a second copy of the product at its own public address reading the dev-test database, which is how this is normally done and which the platform supports natively. Two honest costs, neither hidden: Orbit's model calls cost real money wherever they run, so a preview environment spends from the same ceiling as production unless it is given its own, and the scheduled hourly job would need a deliberate decision about whether it runs there at all. Worth noting what makes this more than convenience: the next slice is email sign-in, and email sign-in is precisely the feature nobody should be testing against real members' identities. Sequencing is the owner's call.
...
- ***Correction, 24 Aug 2026: this is what the item said, and it is not what happened.** The production project was not created. The owner reused his original June development database as production, and the one-shot experiment's database became dev-test. Nothing broke, and the two-databases rule was never crossed, because the two projects really are separate. But two things the deploy record claims rest on a premise that was false: that production started empty, and that the 14 migrations therefore proved the history builds a working database. It also means production went live carrying two months of development fossils, including seed users and `[TEST]` fixture rows, which the owner wiped by hand on 24 Aug 2026. The lesson for the next environment, and it is why this correction sits on the checklist rather than only in the slice entry: **a checklist item that says "create" is not satisfied by "reuse", and nothing on the list asked which it was.*
...
4. **Three settings in the production Supabase project, in this order.** **4.1 and 4.2 DONE 28 August 2026; 4.3 is pasted but still unproven.** Two things the day added that the item did not anticipate. First, **the settings do not sync between the two Supabase projects**, and every one of them had been done on dev-test weeks earlier, which is exactly why they read as already-finished work. Second, **4.1 was already true on production before anyone touched it**, and the evidence was sitting in the owner's inbox: the confirmation code that arrived during QA came from `no-reply@account.interplanetarygroups.com`, an address Supabase's built-in sender cannot send from. A fourth setting joined the list on the day and is recorded here rather than as its own item, because it belongs to the same sitting: **Secure email change switched off on production**, matching dev-test. Left on, an attach needs confirmation from both the old and the new address and returns success with no user and no error, which is the failure the slice's Task 1 spike had already hit once.
   ...
   ***4.3 settled 28 August 2026, on production, by sending: a code arrives, not a link.*** The owner attached an address on the live site, then requested a sign-in code at `/signin` from a second browser, and both mails carried codes that worked. **So the item's one inferred claim is now measured**, and the whole After-launch list is closed. Worth one line on the sequencing, because the item as written could not be followed: **the check had to happen after the merge, not before**, since `/signin` did not exist on the live site until this slice landed. That was safe rather than a corner cut, and the reason generalises: **a wrong answer here cost a dashboard setting with no deploy**, so testing after shipping carried no risk that testing before would have avoided. The rule worth keeping is not "verify before merge" but "know what a wrong answer costs, then choose the moment."

   *What this closes, stated plainly because it is the point of the whole slice:* **a person who is not the owner can now receive a login code from this product.** Both model calls were already proven in production at deploy; this is the third external seam, and the last one the email arc depended on.
...
skipped, zero type errors.** *Declared deviation on length: about 1,140 words against the 400-to-600
...
is deferred rather than blocked, since a conditional write scoped to "not already stamped today"
would satisfy the stamp-after-send rule and the atomicity at once.
...
flow will surface a gap the flow itself hides. **The owner still has not seen the fix rendered by
eye**, on a second pass the same day: he was sent to check it and the QA links themselves crossed
onto a different origin from his session's own, so he never reached `/groups` at all. Same shape as
an existing finding, not a new one; recorded at §8, the hostname-orphaning bullet, where the next
person who hits it should look first.
...
this branch's own suite still carries it red until he does~~ **merged same day as PR #106 (`929a670`).
...
## §11 entry: the group home learns to keep itself current (4 September 2026)

**How it was found, and why that matters more than the bug.** The owner's wife joined a group he had made, posted twice, and Orbit answered her. On his own phone, a minute later, the room was empty. He reloaded and everything appeared. This was the **first time in the product's life that two people used it at once**, the night before he planned to hand it to the tennis group.

Nothing in this product updated an open page. Searched and confirmed absent across `src`: polling, WebSocket, Supabase realtime subscription, and any refetch on focus or visibility. That last search returned zero hits. `revalidatePath`, which the codebase calls everywhere and whose comments read like a live-update mechanism, invalidates the server cache for the **next request** and pushes nothing to anybody's open tab. So a member's own sends updated their own view, navigation re-rendered, and a person who simply sat and watched saw a frozen room forever.

**The correction to the record's own framing.** This was not an architectural mistake. The fix is about thirty lines in one file and the alternative remains additive; nothing about server-rendered pages foreclosed either. What it was is a **QA gap**: every pass ever run on this product was one person on one phone. The decisions that would have been architectural (derived RSVP counts, many-to-many groups from day one, a nullable parent for future event nesting) were all taken correctly and are all recorded above. A missing feature nobody weighed is not the same as a tradeoff somebody chose, and the honest difference is that the Data API was a decision while this never came up at all.

### What shipped

`LiveRefresh`, a render-null client component modelled on `SeenMarker`, calling `router.refresh()` every ten seconds while the tab is visible, plus on tab return, paused while a send is in flight. Mounted in `GroupHome`. Seven decisions settled with the owner before any code, in `docs/superpowers/specs/2026-09-04-chat-sync-design.md`.

**Supabase Realtime Broadcast was weighed and declined, and the reason is not the one first given.** The initial rule-out cited Postgres Changes, which would need the Data API this project deliberately closed. That was the wrong version of the argument: Broadcast can act as a doorbell, pushing a "something happened" ping while the content still arrives through the existing authenticated server render, and no table data crosses the wire. It is the better end-state for a chat product. It was declined on cost against evidence: it ships the anon key to browsers for the first time (safe only *because* the Data API is off, which makes that dashboard setting load-bearing in a new way), and it needs channel authorization that fails **open and silent**. Three new moving parts and a new silent-failure mode, to save a few seconds for a group of eight. **Revisit trigger: members chatting live rather than coordinating.**

**The cheap-check endpoint was dropped as premature.** An earlier plan polled a small "anything new?" query and only refreshed on a change. At eight members a plain refresh is affordable, is ten lines rather than a route plus queries, and automatically keeps the cards correct too, which matters because chip votes and RSVPs post no chat message and **a called-off plan must never keep reading as on**. Kept as a lever if cost appears.

### What review caught, and it is the substance of this entry

Four rounds of independent review found four things worth recording, three of which would have shipped.

**1. A comment claiming a defence that did not exist.** The component wrapped `router.refresh()` in a `try/catch` and its header said this stopped a rejected promise reaching `src/app/error.tsx`. It cannot: `refresh()` returns `void`, so a synchronous catch cannot catch an asynchronous rejection, and the test proving it mocked a *synchronous* throw. Resolved by reading Next 16.3.2's own source rather than reasoning: a network failure during refresh is caught inside Next's fetch layer and converted to a same-URL browser navigation, rethrowing only on an unrelated abort. **So there was no gap; the claim about it was wrong.** The guard stays for the one real synchronous case (dispatch before router initialisation) and the header now says exactly that, with the source cited.

**2. `paused` could stick true forever, silently restoring the bug.** Bound first to `optimisticMessages.some(m => m.isPending)`, which reads the raw `useOptimistic` list. On a hung send, the documented phone-verified case `SEND_DEADLINE_MS` exists for, the request never resolves and never rejects, React never releases the entry, and `paused` stays true for the life of the tab: every future refresh suppressed, the frozen room back, nothing on screen saying so. Now a `sendsInFlight` counter decremented in `.finally()` on the `withDeadline`-guarded promise, which settles within the deadline whatever the network does. The property held is: **once the member has been told the send failed, refresh resumes.**

**The honest limit, stated because a green suite would imply otherwise:** the tests cannot distinguish the two bindings. In jsdom the mocked action carries no real router transition, so both look identical and a future revert would pass CI and fail only in a browser. The tests guard the counter's wiring; the choice between bindings rests on the promise-semantics argument in `GroupHome.tsx`, not on a test. An earlier version of that test file claimed a mutation proof that could not exist; the claim was deleted rather than softened.

**3. This slice introduced a scroll hijack, and its first fix had a hole.** `MessageFeed` auto-scrolled to the bottom whenever the message count changed. That was correct while the count could only change because the *viewer* acted; polling makes it change because *somebody else typed*, so a member re-reading last week's plan gets yanked down mid-sentence. Fixed with a near-bottom guard. **The fix then failed on the empty feed**: the listener effect had empty dependencies and the empty-state branch renders no scroll container, so on a group that starts with no messages nothing ever attached, the near-bottom flag stayed stuck at true, and the hijack returned for the life of the tab. Reachable on the most ordinary path there is, a brand-new group. Now a callback ref, with the empty-start test that was missing.

**4. Four comments elsewhere became false the moment this shipped**, each corrected: `seen.ts` claimed the read position is written on every render (it is written on mount, and there are now six renders a minute); `EmailAskNote` argued its cookie write was bounded because "a member cannot do anything that would revalidate while it is up", which polling makes untrue; and two smaller ones. This project's documented recurring failure is confidently wrong comments, and a slice that changes how often the app re-renders invalidates prose all over the codebase.

**Cleared, so nobody re-derives them:** `SeenMarker` does not re-fire on refresh (no remount, stable deps), so there is no write amplification and no `lastSeenAt` churn. RSVP and chip votes cannot race a poll, because Next runs every action and every refresh through one serial queue. The email-ask sheet's latches hold. The carousel keeps its scroll position.

### Evidence

Baseline on `12ebe2d`: 1730 passed, 146 files. After: 1742 passed, 147 files.

**The browser proof is a real red-to-green in one instrument**, which no test in this repo can supply, since it cannot render server components. With the page open and untouched: a message written straight to dev-test at 01:14:49 **did not appear**, confirmed absent over more than a minute; a single `focus` event then made it appear within 3.5s; a separately timed message posted at 01:16:19 arrived at 01:16:25 by the interval alone, six seconds, inside the ten-second window. **Disclosed:** the Browser pane runs hidden, so `document.visibilityState` genuinely reported `hidden` and the component correctly refused to poll; exercising the interval required overriding that property in the page. That is an instrument override, not a code change. The `focus` path needed none.

**Not verified by anyone:** the mid-send case on a real device, and scroll position on a real phone. Both are the owner's QA, on two devices.

`scripts/qa-stage-chatsync.ts` exists so that stops being a two-person job: it stages a group, and `--say` posts as another member from a laptop while the page under test sits untouched. **The reason this bug reached production is that proving it took two people in one room**, and the owner and his wife are rarely both free. That is now a one-person test, and it is the durable output of this slice.

### Debt

Every visible tab costs a full server render every ten seconds: roughly eight sequential queries plus several identity checks. Affordable at this size and **unmeasured at any other**; the connection pool is the resource to watch, not compute. The poll cost is worth measuring rather than assuming, and this project has been burned once already by a confidently-reasoned mechanism that was wrong.
...
## §11 entry: two test runs, one database, and a lock that deadlocked its own repo (4 September 2026)

*Micro-PR, out of lane, adopted from the safety-net template rather than designed here. Recorded because the process finding is worth more than the change.*

**The failure.** A subagent's own `npm test` ran at the moment `full-suite-on-subagent-stop.mjs` fired and ran the suite too. This project has one dev-test database. The two runs contended for connections until a Prisma transaction could not start, and the suite went red naming `src/lib/groups/__tests__/invite-token.test.ts`, a file nobody had touched. Run alone it was clean. Seen twice in one day by two different sessions; the second sighting named `delete-person.test.ts`, equally innocent.

The misleading name is the whole reason this earns machinery. A red suite pointing at code nobody touched sends whoever reads it to debug something that was never broken, and the owner cannot tell that report apart from a real one. It attacks the baseline number this project's entire verification process rests on.

**What landed.** `suite-lock.mjs` and its test, copied byte-identical from `~/.claude/templates/project-safety-nets/`, plus one line wiring `globalSetup` in `vitest.config.ts`. The files live in `.claude/hooks/` even though this is **not a hook**: that is where `safety-net-drift.mjs` looks, and a file anywhere else reads as missing forever.

**Two choices in it are deliberate and easy to "improve" wrongly.** It is a `globalSetup` rather than a lock inside the stop hook, because a lock only works if every party takes it, and one of the two colliding parties is an agent typing `npm test`, which never calls into a hook; vitest reads its config on every invocation, so both parties take it. And it waits for an in-flight run rather than skipping and trusting it, because the other run may predate the last edit, so its green would not cover the current tree.

**The finding that matters more than the adoption: the template file had never been run against the project it was written for.** Wiring it as written deadlocked this repo. `.claude/hooks/run-tests-unless-docs.test.ts:246` deliberately spawns a nested `npx vitest related --run` in the same working directory, as the one test proving `vitest related` really selects what the per-edit gate assumes. That child tried to take a lock its own parent suite already held, waited the full 300s (measured 301,943ms), threw, and failed the parent's `expect(result.status).toBe(0)`. Parent holds the lock, child waits for the parent, parent waits for the child, on every full-suite run.

The template was made re-entrant rather than this copy being patched, which would have put the two permanently in drift. A run that takes the lock records its path in `CLAUDE_SUITE_LOCK_HELD`; a child inherits it, recognizes this project's own path, and skips acquiring. Keying on the path rather than a bare flag is the part worth keeping: a nested run against a *different* project still locks that one normally.

**The general lesson, and it is about the safety nets rather than about locking.** `safety-net-drift.mjs` can tell you a file is *missing*. Nothing anywhere told anyone the file had never been *executed*. Those are different states and only the second is evidence. A guard shipped on reasoning alone put a deadlock inside the one file whose entire job is making test runs trustworthy, and the drift checker reported it as cleanly adoptable throughout.

**Evidence.** Baseline on `12ebe2d`: 1730 passed, 146 files. After: 1742 passed, 147 files, zero failures, and the formerly deadlocked test green. Duration 71.49s against a 70.89s baseline, so no hidden wait. The +12/+1 is exactly the lock's own test file. **A passing suite is deliberately not offered as the proof**, because the suite passes with the lock absent too: an overlap run showed the lock absent beforehand, held by pid 11317 twelve seconds into a full suite, a second independent run waiting **59 seconds** instead of its usual 0.2s and then **succeeding**, and the lock released afterwards. An earlier attempt reached only "waited, then timed out"; waiting *and* succeeding is the behaviour that had never been demonstrated.
...
## §11 entry: the composers wrap (4 September 2026)

*Micro-slice. Reported by the owner from production with screenshots, the morning after the chat-sync fix landed.*

Both message composers were single-line `<input type="text">`, so a long message scrolled sideways and the member could not see what they had written. The screenshots show `"he line. It just keeps on going forever."` in the group chat and `"also, you need to know where we c"` in onboarding's gap-ask. One cause, two files. Both are `<textarea>`s now, growing with content to about five lines and then scrolling.

**Enter no longer submits, and that was the owner's overrule of this slice's own spec.** The spec, written before any code, settled that Enter would keep submitting because the reported bug was about wrapping rather than about authoring, and a behaviour change nobody asked for is not free. He reversed it from his phone: **Enter inserts a newline and the send button is the only way to send**, which is what iMessage and WhatsApp do, in a product that is mobile-first. It also made the fix smaller, because a textarea does that natively and no key handler exists in either file. **The consequence is worth naming: the send button is now the only exit from both composers**, load-bearing in a way it was not before.

The overrule left the spec document stale, and review caught it: it still read "Enter still submits" and listed "any change to how or when a message is sent" as a **non-goal**, while the shipped change is exactly that. Corrected by striking rather than rewriting, so the document shows where it was wrong instead of quietly agreeing with the outcome.

**A textarea is not a drop-in for an input, and measuring is how we know.** `scrollHeight` excludes the border even under `box-sizing: border-box`, so the first cut left the bordered gap-ask composer 1.5px shorter than the control it replaced. Caught in a real browser rather than in a test, and fixed by adding the element's own computed border back into the height calculation; both quantities are read live, so it survives font-size and zoom changes. Resting heights: **ChatInput 26px before and after, exact; StepGapAsk 43.5px to 44px**, a rounding artifact of the technique, reported rather than smoothed over.

**The fifth test-that-cannot-fail of the week, and this one was kept rather than deleted.** Review proved with a standalone script that **jsdom implements no implicit submit-on-Enter for any form control**, not even a plain input. So the "pressing Enter does not submit" test would pass unchanged against the pre-fix code: on its own it only guards against somebody re-adding an explicit key handler. It was kept because its sibling assertion, that the element is a `<textarea>`, closes the gap — a real browser never implicitly submits one, so the pair is conclusive where neither half is. What changed is the comment, which now says exactly that instead of implying the single test proves the browser behaviour. Deleting it would have removed real coverage of the explicit-handler case; renaming it would have been enough here only because the sibling exists, which is the difference from the suite-lock test deleted the same day.
...
## §11 entry: the deploy already reached the tab, and nobody had ever looked (4 September 2026)

**The request, and why its premise was wrong.** The task came in as "turn on Next's version-skew protection," on the reasoning that `next.config.ts` sets neither `deploymentId` nor `generateBuildId`. The reasoning was sound and the conclusion was wrong: **Vercel supplies the identifier itself.** Three probes against the live site, run before any design work:

1. The production HTML carries `data-dpl-id="dpl_4ifC…"`. Next emits that attribute only when a deployment id is configured, so one is.
2. Every RSC response carries `x-nextjs-deployment-id`, which is set only under the same condition. Independent confirmation.
3. A request carrying a deliberately bogus `x-deployment-id` returns **200, served by the current deployment**, not the 404 Vercel documents for an expired pin and not a pin back to the old build.

Probe 3 is the one that mattered and could most easily have gone the other way. Vercel's Skew Protection exists to *pin* old clients to the old deployment so they keep working, which is the opposite of what was wanted here. Had routing been pinning us, the poll would never have seen a new id and no reload could ever fire.

**So the feature already existed.** Two independent source traces (the second launched specifically to falsify the first) put the chain at `fetch-server-response.js:175` → `doMpaNavigation` → `exitStatus: 2` → `dispatchRetryDueToTreeMismatch` → `server-patch-reducer.js:29` → `completeHardNavigation` → **`app-router.js:221`, `location.replace()`**. Render-phase, no user interaction, no visibility guard, no confirmation. One branch that could have silently swallowed all of it, `segment-cache/cache.js:2250`, was checked and is not on the refresh path: `refreshDynamicData` passes `routeCacheEntry = null`, which gates off every call site that reaches it.

**Which inverts the slice.** The owner's stated risk, "a member could be mid-message when it fires," was not a risk to design against. It has been shipping unguarded since `LiveRefresh` landed the same morning. The work was never to build the reload; it was to make it harmless and to prove it happens.

**Why deferring was rejected, after being the owner's first choice and mine.** The reload is dispatched by Next's own router in response to the refresh's own response. There is no callback and no state we own between the mismatch and `location.replace`. The only lever is whether the poll fires, so "don't reload while they're typing" necessarily means "stop syncing chat while they're typing," regressing the chat-sync slice from the same day and leaving a member who wanders off with text in the box with neither new messages nor the fix. Parking the draft costs less and protects more: it also survives a manual reload, a crash, and an iOS tab eviction.

**The evidence, and it is better than "production only" implied.** Next reads `NEXT_DEPLOYMENT_ID` from the environment into `config.deploymentId` (`server/config.js:826-829`), so building locally with that variable set reproduces the production arrangement exactly, header and all. Confirmed by `BUILD_ID` coming out as the constant `build-TfctsWXpff2fKS`, which Next uses *only* when a deployment id is configured. Four successive builds, `dpl_localtest_one` through `four`, in a real browser at 375x812:

- **Control:** with the tab on `three` and the server restarted onto `four`, the tab did **not** reload on its own. The restart alone is not the trigger. (This mattered: an earlier round's reload turned out to be the preview tooling navigating the tab, not the mismatch. The watched tab was moved to a tab the tooling never touches.)
- **Positive:** one refresh through `LiveRefresh`'s own code path, and the tab performed a full page load, came back on `dpl_localtest_four`, on the same URL, **with the typed draft still in the composer**.
- **Negative:** with tab and server both on `four`, two further refreshes reloaded nothing. This is the direct test of the constant-reload risk the owner named.

**What that does not prove, stated plainly.** The trigger was a `focus` event rather than the ten-second timer, because the Browser pane was hidden and the timer's tick self-gates on `document.visibilityState`. Both call the same `refresh()`; that the timer fires it unattended rests on the chat-sync slice's own QA rather than on this run. And local `next start` is not Vercel. **The honest proof is deploying twice and watching an open tab pick up the second one, and that is still the owner's, after merge.**

**Why a constant reload is unreachable here.** In descending strength: this slice changes no configuration at all, so the identifier keeps coming from Vercel where build-time and serve-time values are the same by construction; the document is never cached (`cache-control: private, no-cache, no-store`), so a reload cannot land back on stale HTML; Vercel promotes production atomically, so two production builds never serve one domain; and `globalMutable.pendingMpaPath` dedupes within a document. Residual, not defended: if a reload ever landed on a still-mismatched build, Next has no cross-reload loop breaker. Nothing above makes that reachable and no guard was built, because the guard's own failure modes would be larger than the thing it defends. The cheap fix, if ever observed, is a `sessionStorage` reload-count cap.

**Debt.** The draft is per tab and clears with the session. **Scroll position is still lost on reload**, which is the only remaining user-visible cost. Screens other than the group home never pick up a deploy at all, since only the group home polls; the owner scoped it there deliberately. And `tsconfig.json` includes `.claude/worktrees/**`, so `next build` typechecks other branches and fails with errors belonging to them: found while trying to run the mandated build on a clean `main`, unblocked by removing two stale worktrees rather than by changing config, at the owner's call. Same class as PR #95's vitest fix, never applied to `tsc`. **Trigger, not a date: the next time a worktree blocks a build**, add it to `exclude`.

### Postscript, same day, from the independent review of the assembled branch

**The reload is not rare, and both documents undersold what the draft parking is for.** The review noticed an adjacency neither the slice document nor the entry above had credited: `fetchServerResponse`'s own `catch` logs "Failed to fetch RSC payload… Falling back to browser navigation." and falls through to **the same `doMpaNavigation` → `location.replace()` path** a deployment mismatch takes (verified directly in `router-reducer/fetch-server-response.js`; the one exception is an aborted request, which rethrows, and a recoverable offline error when `__NEXT_USE_OFFLINE` is on). So **a dropped connection during a poll already hard-reloads an open group home**, which happens a great deal more often than a deploy: a member on patchy signal is the ordinary case, not the exotic one. `LiveRefresh.tsx`'s own header had documented this fallback since it was written; nobody had joined it to what a reload costs. Two consequences worth carrying: parking the draft is a bigger win than this slice claimed, and **a reload fired while the connection is down may land the member on a browser error page rather than on a stale-but-working group home.** That second one is a real product question, it is pre-existing, it is out of this slice's lane, and it is now the strongest known argument for looking at the poll's failure handling.

**Citation corrections to the entry above and to the slice document, made before merge rather than left standing.** `refresh-reducer.js:73` for `navigateType` is `:75`. `ppr-navigations.js:1114` for the `fetchServerResponse` call is `:1111` (the function opens at `:1109`). `stream-ops.node.js:411-427` was cited for "Next emits `data-dpl-id` only when a deployment id is configured"; `:411` is the transform, and the conditional that gates it is `server/stream-utils/node-web-streams-helper.js:769`. The claim that `segment-cache/cache.js:2250` has "five call sites, all prefetch or `routeCacheEntry`-gated" was wrong on both counts (there are six, and only one is so gated); the conclusion stands on the short-circuit in `fetchServerResponse`, which was always the argument that carried it. And the slice document appealed to a "standing rule" about wrapping storage accessors that does not exist anywhere in this repo: this slice is the **first** use of browser storage in the app, so there was no precedent to cite. Each of these is small; recorded together because a record that cites precisely is the only kind worth citing, and this project treats an overclaiming record as a defect equal to a code bug.
...
## §11 entry: the product notices its own deploys, because Next's mechanism never could (4 September 2026)

**Written before this slice's production verification has run, and saying so, because the entry above this one is a correction of a claim made on exactly that kind of confidence.** What is recorded here is what the code does and why; the production numbers get a postscript, and until then **the endpoint being unpinned in a real browser, and the reload actually happening, are unproven.**

**The opening.** Vercel's Skew Protection pins every framework-managed request back to the deployment a tab booted from, so Next's own version-skew reload can never fire here. But a `fetch()` we write is not framework-managed, carries no `x-deployment-id`, and reaches the current build; production also sets no `__vdpl` cookie. Measured, in the three-row table in the correction above. That is the whole design: `/api/deployment` reports the live deployment id, `DeployWatch` compares it against the id the server rendered into the page, and reloads when it is safe.

**When it reloads, which was the owner's decision and not an engineering one.** On tab return and about once a minute while the tab is visible. He chose the periodic check over tab-return-only after asking the question that changed the answer: *what about a member who is actively using the app?* Tab-return-only structurally never reaches them, and an actively-chatting member is the one a bug fix matters most to. The cost is one small request a minute per open visible tab, which at this product's size is a rounding error; the lever if it ever matters is the interval, not the transport, which is why the endpoint is a route handler and not a CDN-served static file (a static asset is the very thing Next appends `?dpl=` to, putting the answer back inside the pinning machinery this routes around).

**The defect review caught, and it would have shipped.** "Safe to reload" was first written as the chat composer being empty with no send in flight. The group home also carries the email sign-in sheet, which holds a typed address and a pending one-time-code step in React state. So: a member opens the sheet, submits their address, switches to their mail app for the code, and comes back — **and returning to the tab is exactly when a check fires.** Composer empty, no chat send, so it reloaded, and they were left holding a code in their inbox with nowhere to type it. That breaks the flow an entire earlier slice was built for, and the tab-return path makes it the likely case rather than an edge one. "Safe" now also covers an open modal and focus sitting in any text field. **The modal check is a DOM query for `[role="dialog"]` rather than a threaded prop, deliberately**: a prop covers only today's sheet, the selector covers every modal nobody has written yet, and a new modal earns the protection by being accessible rather than by somebody remembering a rule. Known limit, named rather than fixed: the chip components dispatch server actions neither signal sees, and a reload mid-tap costs far less, since the vote either landed or it did not.

**The anti-loop protection is one guard, not three, and the record says so because the comfortable sentence would have been wrong.** The reloaded-for record and the per-tab cap both live in `sessionStorage`; they fail together. A read that throws is safe, because the component then does nothing at all. The one remaining route to a loop is a browsing context whose writes appear to succeed but do not survive the navigation, and **nothing in the test suite can reach it**, because jsdom cannot navigate: a module-level variable substituted for `sessionStorage` leaves the relevant test green, which the implementer found and reported rather than letting stand. So the only instrument is a production step, added to the slice document after review: **redeploy twice and confirm the tab reloads once more and then stops.** Steps that redeploy only once pass identically whether the record survives or not.

**One judgement worth keeping, where the subagent was right and the instruction was wrong.** A corrupted negative reload count walked straight past the cap while a comment claimed it could not. The fix offered was to clamp to zero. The implementer instead made a negative count read as *spent*, matching the existing not-a-number branch, on the grounds that a corrupted store is corrupted in both directions and this file's stated principle is to fail toward silence (a stale tab) rather than toward a reload; clamping to zero would have handed a hand-edited store a fresh allowance of three. That is the better answer.

**Evidence.** Suite 1814 → 1867 across 157 files, zero failing. `tsc` clean. Production build exit 0, with `/api/deployment` listed as dynamic rather than static, which is the build confirming the must-never-be-cached constraint. The endpoint was additionally curled through a real `next start`: `200`, `cache-control: no-store`, `{"id":"dpl_localprobe"}`. Every test mutation-proved.

**What local verification could NOT reach, and this is the interesting part.** Running two production builds locally and watching a tab does not test `DeployWatch` at all: with no Vercel in front, nothing is pinned, so **Next's own skew reload fires first** and reloads the tab before our logic gets a turn. Confirmed rather than assumed, by checking that `sessionStorage` held no record — the component writes one before reloading. The two environments are mirror images: locally Next's mechanism works and ours never runs; in production Vercel silences Next's and ours is the only one. **Production is therefore not just the final proof but the only place the integrated behaviour can be observed at all**, including the wait-while-busy rule. What local runs did establish: the endpoint through a real Next runtime, and a clean negative control (tab and server on the same build, two checks, no reload, nothing written).

**Debt.** One request a minute per open visible tab, halved on tab return by coalescing the `visibilitychange`/`focus` double-fire. The per-tab cap can be spent by *failures* rather than by three genuine deploys, after which that tab behaves exactly as it did before this slice, silently. Scroll position is still lost on any reload. Only the group home checks, so a member sitting on event detail or group info gets nothing. And a real-phone pass is owed on the sheet interaction, since the modal and focus guards behave against a live keyboard in ways jsdom cannot show.

---

## §11 entry: two Mikes get caught at the door (4 September 2026)

**The problem was confusion, not incorrectness, and the record had to say so first.** The product collects only a first name, so two members called "Mike" render as the same word on ten screens: identical initials on identical grey circles in the roster, "Mike · Mike" on group info, "Mike & Mike are in so far" from Orbit. An earlier reading had implied counts could go wrong. They cannot: every vote, RSVP, tally and digest keys off the member id, so two Mikes are counted correctly everywhere. Naming that precisely is what let the fix be small.

**Why at the door rather than on the screens.** Ten render sites share no common seam, so a display fix would be ten fixes. Worse, two kinds of text bake a name in at write time and carry no id: the join announcement ("Mike joined") and cancellation lines. No later display fix can repair those, so the only place a name can be made correct for good is the moment it enters the product. Catching it at join means every stored string is right from birth.

**What was settled, and it is the owner's.** A collision is refused, compared case-insensitively and whitespace-trimmed, so "mike" and "Mike " both collide. **The person cannot decline**: two Mikes forever is worse for the group than one moment of friction. The copy offers both doors, because a collision is the strongest signal this product has that somebody is a returning member who lost their session: *"There's already a Mike in this group. If that's you, sign in instead. If not, add a last initial so people can tell you apart."* Short, because vertical space on that screen is contested. **Not at group creation**: there is nobody to collide with when the group does not exist yet, and that reasoning was verified against the code rather than taken on faith, since `provisionFounderGroup` writes the group and the founder's membership in one statement.

**The case the settled decisions did not reach, and the owner's ruling on it.** Two populations tap an invite link. Somebody with no session types a name, and the check gates it. Somebody who already has a session, joining a second group, sees a read-only "Joining as Jacob" and has no name field at all, because `joinGroupByInvite` deliberately never overwrites a stored name. Enforcing the block there meant either letting them edit a name that lives on `User`, renaming them in every group retroactively, or refusing the join with no remedy anywhere in the product. **The owner's call, made during the build: let them in.** The audience is small and the collision is unlikely, and a dead end on the activation screen is the worse failure. So the check gates the name at the one moment it enters the product, which is `User` creation.

**That also keeps the copy's own promise true, by construction.** `confirmJoinSignInAction` joins with an empty name on an existing `User`, so somebody recovering their session can never be blocked by the very check that told them to sign in. The check sits on the `User`-creation branch alone, and a test pins that asymmetry so a future session cannot tidy it away.

**What happens if two Jacobs does become a real problem: they change their name universally, across every group they are in.** The owner ruled that out for now and queued it. A per-group display name was weighed and **rejected on its own terms**, not deferred: it is a second source of truth for a person's name and something for everybody to keep track of, and it would need a migration plus every one of the ten render sites, which is the exact cost this slice was designed to avoid. The universal rename is roughly a day and this slice makes it easier rather than harder, because the check and the error state already exist; the change is to render the field for a colliding session-bearing joiner and let the write through. **The caveat to carry into that work is the same one that shaped this slice:** a rename fixes every screen and does not fix stored text, so the old join line would still read "Jacob joined" while every bubble reads "Jacob R", and `deletion-plan.ts`'s literal-body match would then stop finding a renamed person's join line at all.

**One thing on the copy that is a rule rather than a detail.** "SIGN IN" is a real inline button inside the sentence, not prose, and it opens the panel the "I've been here before" link at the foot of the form already reaches. Two routes to one door is intended. It is **not teal**: teal is a weight in this product and the weight on that screen belongs to joining. It also deliberately does not take the 44px tap-target floor, because it is inline text inside a sentence and a box would break the line.

**The owner's phone QA changed that copy, and the reasoning is worth more than the words.** He could read the sentence and still kept sliding past the control inside it. His fix works from both ends at once rather than just shouting: the control goes to **caps**, which is the only emphasis lever left once teal is spent on joining and the sentence is already `--danger` red, **and the sentence around it gets shorter so there is less competing with it.** "in this group" went because the card directly above already names the group; "so people can tell you apart" went because the first sentence has already said there is another one of you. Final copy: *"There's already a Mike here. If that's you, SIGN IN instead. If not, add a last initial."* **It also did something he expected it not to: the error dropped from three rendered lines to two at 375x812**, measured, which is real space back on a screen this project treats as contested. Two notes for whoever copies this pattern: the caps here are **not** the product's uppercase eyebrow style, they are two words inside a running sentence at the sentence's own 15px; and shortening the copy was doing the same job as capitalising it, which is the transferable half.

**This is the product's first `aria-invalid` / `aria-describedby` pairing anywhere in `src/`, so it is a precedent and was treated as one.** `role="alert"` was tried and then deliberately removed: an assertive live region containing an operable control is a known screen-reader weak spot, and as a *description* on the input the words "sign in" would be announced as description text with nothing marking them operable. The tradeoff is real and is recorded at the wiring, because a future session must not restore the role without knowing why it went: without the live region the error may not be announced at the instant it appears, only when focus reaches the input. **Unverified against an actual screen reader**, and stated that way rather than claimed.

**Evidence.** Baseline before the branch was 1814 passing across 153 files, and 1827 across 154 after, run first-hand rather than copied from a report. `npx next build --webpack` exits 0, which is a gate this project added after three deploys failed on 4 September while four suites stayed green. A browser pass at 375x812 walked the whole arc: the collision refused with the stored casing shown, the inline control opening the sign-in panel, "I'm new here" returning, a differentiated name joining, and the feed announcement carrying the differentiated name. `scrollHeight === innerHeight === 812`, measured rather than eyeballed, so the three-line error grows the screen without pushing the join button or the consent line off it.

**What automated verification could not reach, named rather than glossed.** Reaching the sign-in panel *through* the collision is a genuinely new state: the join action mints an anonymous Supabase session before the check throws, so the sign-in door is now reachable while holding a session, which it never was before. Reading the code, `verifyOtp` replaces the session cookie and this works; that is confident, not certain, and it is the first item on the QA script because the failure it would produce is exactly the one the design set out to avoid, the product telling somebody to do something that then does not work.

**Debt this opens.** The check is read-then-write inside a transaction rather than a database constraint, so two people with the same name tapping join in the same instant could both get in; a real constraint would need `name` denormalised onto `Membership` with a unique index, which is a migration and a second source of truth for a person's name, declined against a race needing two same-named strangers inside one transaction's width. A session-bearing joiner can still collide, per the ruling above. And a refused collision now leaves an orphaned Supabase `auth.users` row, because the anonymous identity is minted before the throw and only the app-side transaction rolls back: the shape is pre-existing for any post-session failure, but the collision turns it from an error path into a routine one, and those identities are invisible to `npm run person:delete`, which works off `User`.

**Not this slice's, and still open:** `src/lib/people/deletion-plan.ts` matches join announcements by literal body text product-wide, so deleting one Mike surfaces the other's line as a high-confidence candidate the runbook says is safe to confirm. Recorded at build-notes 6852. This slice makes new collisions impossible and does nothing for the ones already stored. Its own item. *(**That bug produced evidence for itself during this slice, unprompted, and the record should upgrade accordingly.** It has been carried as a reasoned-about hazard: nobody had watched it mis-fire. The baseline suite run failed one assertion in `delete-person.test.ts` because a **concurrent worktree's** `[TEST] … joined` rows matched by body text **across group boundaries** and were returned as candidates for a person in a different group entirely. It passes clean in isolation, so what happened here is shared-database contamination rather than a product defect. The mechanism is the point: the query is unscoped, and given two people who share a name it will hand the runbook another person's line as a high-confidence candidate. Confirmed by two sessions independently on 4 Sept 2026.)*
