...
- **Two helpers were built ahead of their consumers.** A "current user" lookup and a browser-side Supabase client exist but are not yet used; they were built here because the Supabase plumbing was already in context, and they are flagged so the member-join and group-surface slices either consume them or prune them, rather than leaving them as silent dead code.
