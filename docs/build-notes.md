...
- **UI** — Step 2 gains an always-on quiet standing-place input beneath each rhythm's value line; Step 1 ships the mockup's hint line below Continue, extended to invite the spot, and the placeholder example names one.
...
- **Step 2 treatment (open question b):** a quiet second line under its rhythm's row, not a standalone WHERE row — implemented as an always-on input (the group-name row precedent, confirmed always-on by reading the component, styled quieter at label scale). Approved extension of the spec: captured venues are editable before confirm, so a mis-extraction is fixable without going back to Step 1. **The empty state is deliberately neutral, never lime**: lime on the onboarding cards means "Orbit needs this before it can proceed," and venue never blocks, so borrowing the marker would teach founders the lime cue is sometimes ignorable.
...
- **Step 1 hint copy:** the mockup line ("Orbit reads this to set your days, send reminders, and build a shared group page.") ships verbatim in the mockup's position below Continue, with one added sentence: "Mention your usual spot too, if you have one." Finding recorded for honesty: the line existed only in the mockup, not in shipped code, and the walkthrough export is a JS-packed blob that text greps cannot inspect — an earlier "not in the mockup" claim was wrong for exactly that reason; the file must be rendered in a browser to verify copy.
...
**QA data note.** Browser verification created three groups in the dev-test DB via the real wizard (two "Sunday Climbers" — one venued, one not — and "Saturday Yoga"; the gap-round spot check stopped before confirm and created nothing). Left in place as the PR's inspectable evidence; expendable dev data thereafter, deletable at the owner's discretion.
