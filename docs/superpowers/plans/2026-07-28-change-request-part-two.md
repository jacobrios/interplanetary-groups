...
**Branch state:** you are already on `feat/change-request-consensus`, cut from a current `main`, with the design spec and this plan committed on it. Do not cut a new branch and do not branch from main; nothing is pushed yet. Task 15's final step pushes and opens the slice's one PR (docs and code together). Suite baseline at branch time is 34 files / ~450 tests green.
...
investigation so this session's context stays clean.

BRANCH STATE (do not cut a new branch, do not branch from main): you are
already on feat/change-request-consensus, cut from a current main, with the
spec and the plan committed on it. Nothing is pushed yet; Task 15's final step
pushes and opens the slice's one PR.

Work task by task with TDD as the plan specifies. Do not re-plan and do not
re-litigate settled decisions. The dev-test sandbox is dirty from part-one QA;
Task 14 resets it before any browser verification and requires a two-plan
sandbox. Open the PR at the end with the manual QA script and stop; do not
merge.
```
