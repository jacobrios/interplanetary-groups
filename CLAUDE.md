...
**Recognition no longer eats a direct ask before the ladder sees it, and there is now a bench that can prove it.** The never-silent rule had been written into the reply ladder but not into the recognition step upstream of it, so a bare "can we move it?" was classified as not-a-request and died in silence. Three fixes closed it: the intent prompt no longer lists a question asking for a change alongside questions that only want information, its tiebreak now splits "unsure anyone is asking anything" (still stay quiet) from "unsure only what they meant" (say so and let the ladder ask), and a guard in the claim-to-fact boundary that threw away any change request naming no field is gone. `npm run eval:detect` runs the recognition bench: twenty-two cases, each a short setting plus one message plus the outcome a member should see, graded through the real prompt, the real normalize layer, and the real ladder, with no database. It is deliberately not part of the test suite, because it costs money and hits the network. Baseline and after-numbers are in build-notes §11; the asks went from 26/50 runs to 50/50 and the messages Orbit must not answer held at 45/45.

**Next slice: the wrong-day retry.** The endgame handles an idea nobody got to; it does nothing for an idea people clearly want on a day that does not work. Today that gauge just closes. Open questions to settle before it can be spec'd:
- **How does Orbit pick a different day with no signal about which day works?** A no on the chips says nothing about what would have worked, and simply guessing again is close to the conflict-aware proposing that is explicitly out of scope.
- **It needs a deliberate amendment to "a gauge whose day has passed is dead."** That line is settled and this slice would bend it, so bending it is the spec's first job rather than something discovered mid-build.

Queued as rivals for the slice after: a pending-events surface (somewhere to see unanswered ideas without scrolling) and letting Orbit answer read-only questions like "what's open?", which may be the cheaper of the two and should be weighed head to head.

Settled and not open, carried forward: the three-person threshold; below-threshold ideas scrolling away with no residue; **the initiator counted only when they named the day Orbit is proposing** (their message already is that yes, seeded at gauge creation), voting like anyone else when Orbit picked the day; a day counting as named only when exactly one is named; both flavours of no seeding OUT on a created event; a gauge closing two hours before its proposed start, at the start itself for a gauge born inside that window (amended 4 Aug 2026, gauge-endgame slice); zero-yes ideas scrolling away silently while a gauge with at least one yes closes with a single chip-less note (amended 4 Aug 2026, gauge-endgame slice); and a threshold reached after the proposed start creating nothing. Reasoning in build-notes §11, spark parts one and two.
