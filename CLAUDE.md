...
**Next slice: change request part two (consensus and a conversational window).** Reprioritized ahead of one-bump after the owner's post-merge QA (28 July 2026; findings and decisions in build-notes §11, part-two seed). Its scope, already decided: the consensus rule replaces the unilateral move (a time change is a proposal; at least three yeses counting the asker, and more yeses than the current time has people in); detection reads a recent-messages window that includes Orbit's own messages and message timestamps, fixing the wrong-target, dead-correction, and ambiguous-"it" failures QA surfaced; and a direct ask with an ambiguous part gets a verifying question, never silence, per the amended guardrail. Open before spec: the small-group edge where beating the incumbent is arithmetically impossible, and the window's exact size.

**After it: the one-bump resurface.** Orbit's "one bump, then let it die" guardrail is written but not built: a stalled-but-viable idea earns at most one resurfacing, no pinning, no banners. Open questions to settle before it can be spec'd:
