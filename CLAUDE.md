...
- **Normalize model output before it drives control flow.** Orbit's structured extraction returns raw model output, and raw output is a claim about the world, not a fact. Validate and normalize it against the schema before any branch reads it. A concrete case from the one-shot experiment: extraction reported a field as missing that the schema did not require, which would have triggered a clarifying question to the founder about something the product never needed to know. Every user-facing behavior that keys off an extraction result reads the normalized shape, never the raw response.
...
- **Two databases, never crossed.** Production and `interplanetary-groups-dev-test` are separate Supabase projects with separate credentials. Never point a production build, migration, or seed script at the dev/test database, and never the reverse. If a task seems to require it, stop and ask.

---

### How the build agent should work

- **Ask or flag before assuming on anything that shapes product behavior, requirements, or architecture.** On pure implementation detail, use your judgment and pick the simplest thing that works, but record any notable choice (a line in the PR, and in build-notes §11 if it is a lasting decision) so the reasoning is visible later. Rule of thumb: if a future reader would ask "why was it done this way," either ask first or write it down. **A design element with no obvious home in the data model is always one of these.** If a screen implies data that no field currently holds, say so and ask where it lives. Do not build the parts that have a home and leave out the part that needs a decision. (This deliberately preserves "prescriptive on the what, open on the how." It is not a mandate to stop and ask about everything.)
...
- **Claims about behavior need artifacts, not assertions.** Anything you report as working, as matching a design, or as fixed must come with evidence a reader could check: a screenshot of the rendered screen, raw command output rather than a summary of it, a stored database value next to the displayed one, or a test shown failing before it passed. Use judgment about which artifact fits the change, never about whether one is produced. If you cannot produce it, say so plainly and name what would be needed. "Verified" with nothing attached is a status report, and this project does not accept status reports as evidence. The obligation attaches to the claim, not to the slice: a slice that makes no behavioral claim owes no artifact, and a slice claiming four visible behaviors owes four, so refactors and documentation work pay nothing. (Lineage: the one-shot experiment findings in build-notes §11.)
- **A passing test is only evidence if it could have failed.** Write the test first and show it failing before the fix. When a test passes on its first run, say why it could have failed, and confirm that nothing environmental (the machine's timezone, locale, or clock) is what made it green.
...
- **Start every new slice from a current main.** Before creating a branch for a new slice, check out main, pull, and confirm the local main matches origin. If the previous slice's branch still exists locally after its PR merged, delete it as part of this step. Never branch from a stale main or from a previously merged feature branch. Rationale: the human merges PRs in GitHub, which deletes the remote branch but leaves the local checkout stale; making freshness a start-of-slice precondition removes the dependence on a manual post-merge pull.
